...
public class Main {
    ...
    static void getInitials(String x) {
        ...
        for (int i = 0; i < x.length(); i++) {
            String letter = String.valueOf(chars[i]); // метод '.valueof()' возвращает тот тип, который указан
                                                      // перед точкой.
            if (!letter.equals(letter.toLowerCase())) itog.append(letter);
        }
    ...
    public static String botHelper(String x) {
        int index = x.toLowerCase().indexOf("help");
    ...
    public static boolean isAnagram(String x, String y) {
        char[] chars_x = x.toLowerCase().toCharArray();
        char[] chars_y = y.toLowerCase().toCharArray();
        Arrays.sort(chars_x);
        Arrays.sort(chars_y);
